...
> **Arquivo:** `src/components/CadastroLayout.tsx` · **Diretiva:** `"use client"` · **Linhas:** ~95

---

## Propósito

Componente que organiza o cadastro em **3 abas**: Cadastro, Evolução e Histórico. Contém o header da seção e gerencia a navegação entre as abas. Marcado com `"use client"` pois usa `useState`. A navegação principal (Cadastro, Agenda, Financeiro) é feita pela `TopBar`.
...
> **Nota:** O background gradiente foi movido para `page.tsx` na reestruturação da Top Bar.
...
- A aba **Agenda** foi movida para a **Top Bar** como navegação principal
